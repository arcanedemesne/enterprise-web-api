...
namespace Enterprise.Solution.API.Application.Handlers
{
    ...
    public class PatchBookHandler : BaseHandler<PatchBookHandler>, IRequestHandler<PatchBookCommand>
    {
        ...
        {
            _logger.LogInformation($"{nameof(PatchBookHandler)} constructor invoked.");
        ...
        {
            _logger.LogInformation($"{nameof(PatchBookHandler)} handler invoked with request type {nameof(PatchBookCommand)}.");
            ...
            _logger.LogInformation($"Try to apply patch {nameof(Book)} with id {request.Id} to model state.");
            ...
            try
            {
                _logger.LogInformation($"Try to update {nameof(Book)} with id {request.Id} using ops & values from patch command.");
                await _service.UpdateAsync(entity);

            }
            catch (Exception ex)
            {
                var nEx = new NotPatchedException(typeof(Book), request.Id);
