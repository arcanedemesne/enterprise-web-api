...
namespace Enterprise.Solution.API.Controllers
{
    ...
    public class AuthorController : BaseController<AuthorController>
    {
        ...
        /// <param name="queryParams">IncludeBooks || IncludBooksWithCovers || IncludeBooksWithCoversAndArtists</param>
        /// <returns code="200">Found Author</returns>
        [HttpGet("{id}", Name = $"Get{nameof(Author)}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<IActionResult> GetByIdAsync(int id, [FromQuery] AuthorQueryParams queryParams)
        {
            try
            {
                var dto = await base._mediator!.Send(new GetAuthorByIdQuery(id, queryParams));
                return Ok(dto);
            }
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<IActionResult> AddAsync([FromBody] AuthorDTO_Request dto)
        {
            try
            {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<IActionResult> UpdateAsync(int id, [FromBody] AuthorDTO_Request dto)
        {
            try
            {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<IActionResult> PatchAsync(int id, [FromBody] JsonPatchDocument<AuthorDTO_Request> jsonPatchDocument)
        {
            try
            {
